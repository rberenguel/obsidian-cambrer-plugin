class MetaP {
	...
	selectCommand(command, div) {
		...
		if (command.inputs && Array.isArray(command.inputs)) {
			Array.from(this.commandListContainer.querySelectorAll("DIV")).map(
				(d) => {
					if (d != div) {
	...
	_updateSearchInputDisplay() {
		...
		this.searchInputDisplay.style.display = this.searchText
			? "block"
			: "none";
	}
	...
	_renderVisibleCommands() {
		this.commandRows.forEach(
			(rowObj) => (rowObj.div.style.display = "none"),
		);
		this.commandListContainer.innerHTML = "";
	...
	_handleEllipsisDisplay() {
		...
		if (
			...
			if (!this.ellipsisRow) {
				...
				this.ellipsisRow.classList.add(
					"metap-modal-row",
					"metap-ellipsis-row",
				);
