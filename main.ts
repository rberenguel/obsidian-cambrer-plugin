...
export default class LocalServerPlugin extends Plugin {
	...
	async onload() {
		...
		if (this.app.vault.adapter instanceof FileSystemAdapter) {
			this.pluginPath = this.app.vault.adapter.getFullPath(
	...
	startServerAndOpenPage() {
		if (!this.server) {
			...
			try {
				...
				new Notice(
					`Error starting server. Is port ${this.settings.port} in use?`,
				);
	...
	/**
	 * Extracts all active stylesheets from the document head.
	 */
	getCombinedStyles(): { themeClass: string; allCss: string } {
		...
		for (const sheet of Array.from(document.styleSheets)) {
			if (sheet instanceof CSSStyleSheet) {
				try {
					...
					console.warn(
						`Cambrer: Could not read CSS rules from stylesheet: ${sheet.href}`,
						e,
					);
				}
			}
	...
	postProcessHtml(filePath: string, html: string): string {
		...
		openInObsidianLink.style.cssText =
			"position: fixed; top: 10px; right: 10px; padding: 8px 12px; background-color: var(--background-secondary-alt); color: var(--text-normal); text-decoration: none; border-radius: 5px; font-family: sans-serif; font-size: 14px; z-index: 9999; border: 1px solid var(--background-modifier-border);";
	...
	serveAsset(filePath: string, res: http.ServerResponse) {
		...
		try {
			...
			const fileExtension = path
				.extname(filePath)
				.toLowerCase() as keyof typeof MIME_TYPES;
			const contentType =
			...
			console.error(
				`Error serving asset ${filePath} from ${fullPath}:`,
				error,
			);
	...
	createServer() {
		this.server = http.createServer(async (req, res) => {
			...
			const filePath = decodeURI(
				theReqUrl.startsWith("/") ? theReqUrl.substring(1) : theReqUrl,
			);
			...
			if (!(file instanceof TFile)) {
				res.writeHead(404, { "Content-Type": "text/html" });
				res.end(
					`<h1>404 - Not Found</h1><p>Could not find file: ${filePath}</p>`,
				);
				return;
			}
			...
			try {
				...
				if (fileExtension === ".md") {
					...
					const processedHtmlBody = this.postProcessHtml(
						file.path,
						container.innerHTML,
					);
					const { themeClass, allCss } = this.getCombinedStyles();
					...
					res.writeHead(200, {
						"Content-Type":
							MIME_TYPES[
								fileExtension as keyof typeof MIME_TYPES
							],
					});
					...
					res.end(
						`<h1>415 - Unsupported Media Type</h1><p>Cannot serve file type: ${fileExtension}</p>`,
					);
				}
			} catch (e) {
...
class LocalServerSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Server port")
			.setDesc(
				"The port for the local server. The server will restart on the new port the next time you serve a note.",
			)
			.addText((text) =>
				text
					.setPlaceholder("8123")
						...
						if (!isNaN(port) && port > 0 && port < 65536) {
							if (port !== this.plugin.settings.port) {
								...
								if (this.plugin.server) {
									...
									new Notice(
										"Server stopped. It will restart on the new port next time you serve a note.",
									);
								}
							}
						}
